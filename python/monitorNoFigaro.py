...
def figaroMonitor(filename, runtime, port=port, baudrate=115200, figaro=True, bme=680, scd=False):
    ...
    with open(filename, 'w', newline='') as csvfile: # generate CSV to store data
        ...
        print(fieldnames)
        readoutDict = dict.fromkeys(fieldnames)
        while True:
                b = arduino.readline() # get serial output
                s = b.decode() # generate string from serial output
                if len(s) != 0 and s[:2] == "x": # don't start logging until the beginning of a complete chunk of data
                    break
        while t - t0 <= runtime: # note: Arduino reports absolute time in milliseconds
            for k in range(len(fieldnames)):
                ...
                s = b.decode()
                print(s)
                if len(s) != 0 and s[:2] in fieldnames: # prevents Python from trying to record empty lines in serial output
...
figaroMonitor('null2.csv', 0.25, port=port, figaro=False, bme=680, scd=False)